...
import { MultiSelect, type MultiSelectRef, type Option } from "@/components/ui/multi-select";
import { type ParentSkills, type Skills, skillSubSkillMap, type SubSkillsType } from "@/interface/skills";
...
export const SkillsSelect = React.forwardRef<MultiSelectRef, SkillsSelectProps>(
  ({ defaultValue = [], onChange, className, maxSuggestions = MAX_SUGGESTIONS, formMessage }, ref) => {
    const [uiSkills, setUiSkills] = React.useState<Option[]>(convertSkillsToOptions(defaultValue));
    ...
    const transformToBackendSkills = React.useCallback((options: Option[]): Skills => {
    ...
    const getSuggestions = React.useCallback((): Option[] => {
      ...
      const addSuggestion = (option: Option) => {
        if (!selectedValues.has(option.value) && suggestions.length < maxSuggestions) {
          suggestions.push(option);
        }
      };

      const selectedParentSkills = new Set(logicSkills.map((skill) => skill.skills));
      ...
      logicSkills.forEach(({ skills: parentSkill, subskills }) => {
        const unselectedSubskills = skillSubSkillMap[parentSkill]
          .filter(({ value }) => value !== "Other" && !subskills.includes(value as SubSkillsType))
          .slice(0, 4);
        ...
        complementarySkills.forEach((compSkill) => {
          if (!selectedParentSkills.has(compSkill)) {
            ...
            const unselectedSubskills = subskills
              .filter(({ value }) => value !== "Other" && !selectedValues.has(`${compSkill}:${value}`))
