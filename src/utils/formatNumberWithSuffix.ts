export const formatNumberWithSuffix = (amount: number, decimals: number = 2, skipThousands: boolean = false) => {
  ...
  if (tier === 1) {
    ...
    formattedNumber = scaled % 1 === 0 ? scaled.toLocaleString("en-us") : scaled.toFixed(1).replace(/\.0$/, "").toLocaleString();
  }
